...
# Reads from environment: COSMOS_ENDPOINT, COSMOS_KEY, GH_TOKEN
...
# Gather open issues count
OPEN_ISSUES=$(gh issue list --repo Copilotclaw/copilotclaw --state open --json number 2>/dev/null | python3 -c "import sys,json; print(len(json.load(sys.stdin)))" || echo "unknown")
PRIORITY_NOW=$(gh issue list --repo Copilotclaw/copilotclaw --state open --label "priority/now" --json number 2>/dev/null | python3 -c "import sys,json; print(len(json.load(sys.stdin)))" || echo "0")
PROPOSALS=$(gh issue list --repo Copilotclaw/copilotclaw --state open --label "crunch/proposal" --json number 2>/dev/null | python3 -c "import sys,json; print(len(json.load(sys.stdin)))" || echo "unknown")
